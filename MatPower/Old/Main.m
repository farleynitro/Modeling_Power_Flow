...
%PV farm
  ...
  N_daily_dispatch = 12; %total available wind profiles
  ...
  A_pv = 181112.1722; %Area in m2 of each PV system , a total of 4 PV
...
%     for(j=1: N_daily_dispatch) %makes a 12x1
...
%   for(j=1: N_daily_dispatch) %makes a 12x1
%     samples_Q(j,i)= normrnd(Q_mean(i), 0.75*Q_std(i), [1, 1]); %Creates random samples for Q in 12x13, some values violating the constraints
%   end
% end
% 
...
%   for(j=1: N_daily_dispatch) %makes a 12x1
%     samples_Q_pv(j,i)= normrnd(Q_mean_pv(i), 1*Q_std_pv(i), [1, 1]); %Creates random samples for Q in 12x4, some values violating the constraints
%   end
% end
