...
for(i=1:N_strings) %makes a 1x13
  for(j=1:N_daily_dispatch/2) %makes a 96x1
    samples_Q(j,i)= normrnd(Q_mean1(i), Q_std(i), [1, 1]); %Creates random samples for Q in 96x13, some values violating the constraints
  end
    for(j=(N_daily_dispatch/2):N_daily_dispatch)
    samples_Q(j,i)= normrnd(Q_mean2(i), Q_std(i), [1, 1]);
  end
