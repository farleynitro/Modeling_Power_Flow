...
%Last updated: 20/05/2020 (dd-mm-yyyy)
...
N_daily_dispatch = 96; %total available wind dispatch profiles for 3 hours
...
Q_mean1 = zeros(1, N_strings);%matrix with mu for each string 1-13
Q_mean2 = zeros(1, N_strings);
Q_mean3 = zeros(1, N_strings);
...
samples_Q = zeros(length(Q_mean1),1); %initialize matrix for Q random samples

for i=1:length(Q_mean1)
...
for(i=1:N_strings) %makes a 1x13
  for(j=1:N_daily_dispatch/3) %makes a 96x1
    samples_Q(j,i)= normrnd(Q_mean3(i), 0.1*Q_std(i), [1, 1]); %Creates random samples for Q in 96x13, some values violating the constraints
  end
    for(j=round(N_daily_dispatch/3:(2*N_daily_dispatch/3)))
    samples_Q(j,i)= normrnd(Q_mean1(i), Q_std(i), [1, 1]);
  end
    for(j=round((2*N_daily_dispatch/3):N_daily_dispatch))
    samples_Q(j,i)= normrnd(Q_mean2(i), 0.1*Q_std(i), [1, 1]);
  end
end
...
while(stop == 0)
   ...
   %for (k=1:N_branches_shunt)
        ...
        constraints_v_low = Vmag(Vmag < 0.9); %Note values greater than minimum p.u.
        constraints_v_high = Vmag(Vmag > 1.10);  %Note values greater than maximum p.u.
...
for (a=1:N_daily_dispatch)
        ...
        idx_constraints_v_low(:,a) = (Vmag(:,a) < 0.9); 
        idx_constraints_v_high(:,a) = (Vmag(:,a) > 1.10);
