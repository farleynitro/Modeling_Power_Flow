...
%     samples_Q(j,i)= normrnd(Q_mean(i), 1*Q_std(i), [1, 1]); %Creates random samples for Q in 96x13
%   end
% end
...
% 
%    for(k=1: N_strings)
%     constraints_neg = samples_Q(samples_Q(1,:) < -Q_gen_max(k)); %Note values greater than positive max reactive 
%     constraints_pos = samples_Q(samples_Q(1,:) > Q_gen_max(k));  %Note values greater than negative max reactive
%     
%     idx_constraints_neg = (samples_Q(1,:) < -Q_gen_max(k)); %Note values greater than max reactive 
%     idx_constraints_pos = (samples_Q(1,:) >  Q_gen_max(k));
%     
