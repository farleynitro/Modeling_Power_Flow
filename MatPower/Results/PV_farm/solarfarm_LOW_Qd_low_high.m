...
mpc_substation_system13.bus(1,3) = 50.5272; 
mpc_substation_system13.gen(2:5,2) = 12.6318;
...
%PV farm
  ...
  N_daily_dispatch = 600; %600 since dispatches are divided by 3 ! total available solar profiles
...
for(i=1:N_strings_pv) %makes a 1x13
  ...
  for(j=(3*N_daily_dispatch/10):(4*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv4(i), a*Q_std_pv(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(4*N_daily_dispatch/10):(5*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv5(i), a*Q_std_pv(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(5*N_daily_dispatch/10):(6*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv6(i), a*Q_std_pv(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(6*N_daily_dispatch/10):(7*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv7(i), a*Q_std_pv(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(7*N_daily_dispatch/10):(8*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv8(i), a*Q_std_pv(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(8*N_daily_dispatch/10):(9*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv9(i), a*Q_std_pv(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(9*N_daily_dispatch/10):(N_daily_dispatch))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv10(i), a*Q_std_pv(i), [1, 1]); %Creates random samples for Q 
  end
end
...
while(stop == 0)
   ...
   %for (k=1:N_branches_shunt)
        ...
        mpc_substation_system13.bus(1,4) = sum(samples_Q_pv(i,j));
...
%end 
       ...
       xlabel('Dispatch','FontSize',12);
        ...
        plot3(Q_tot, P_loss_tot, v_avg,'o','Color', 'k', 'MarkerSize',5); %plot for avg
        hold on;
        ...
        legend('100 W/m2','500 W/m2', '800 W/m2');
        
        %Plot 3D plot Q loss for minimum and average voltage
        figure(4)
      
        plot3(Q_tot, Q_loss_tot, v_avg,'o','Color', 'k', 'MarkerSize',5); %plot for avg
        hold on;
        ...
        plot3(Q_tot, Q_inj_tot,v_avg,'o','Color', 'k', 'MarkerSize',5); %plot for avg
        hold on;
