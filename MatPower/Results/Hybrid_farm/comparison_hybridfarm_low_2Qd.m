...
% type_wtg= 4; %total types of WTG
  ...
  for (i=1:N_daily_dispatch) 
    v_w(i,:) = 4; %nominal wind speed in m/s
  end  
...
%    end
  ...
  P_pv_max = [12.63168  12.63168  12.63168  12.63168];  %800%Avg active power in MW generated per PV string 1-4
  P_pv_min = [1.57896   1.57896   1.57896   1.57896]; %100
  P_pv_nom = [7.8948    7.8948    7.8948    7.8948]; %500
...
for(j=1:N_strings)
for(i=1:(N_daily_dispatch))
    if(i <= (4*N_daily_dispatch/12))
    Q_mean_wt1(i,j) = -9;
    elseif ((i > (4*N_daily_dispatch/12)) && (i <=(8*N_daily_dispatch/12)))
...
for(i=1:N_strings) %makes a 1x13
  for(j=1:(4*N_daily_dispatch/12)) %makes a 96x1
  ...
  for(j=(4*N_daily_dispatch/12):(8*N_daily_dispatch/12))
...
%         Q_inj_tot_v_high(numel(Q_tot_constraints_v_high)) = 0;
        ...
        figure(9)
        plot(v_avg, P_loss_tot, 'o');
        hold on;
        %plot(constraints_v_lowest,P_loss_tot_v_low, 'o');
        hold on;
        %plot(constraints_v_highest, P_loss_tot_v_high, 'o');
        hold on;
        
        grid on;
        
        xlabel('V average magnitude (p.u.)');
