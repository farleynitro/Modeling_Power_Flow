...
% type_wtg= 4; %total types of WTG
  ...
  P_wt_max = [29.4 29.4 29.4 16.8 32 29.4 16 28 33 29.15 28 28.8 28.8]; %Rated power in MW for each string
  ...
  P_pv_max = [12  12  12  12];  %Max active power in MW generated per PV string 1-4 
...
%    end
  ...
  P_pv_max = [12  12  12  12];  %Avg active power in MW generated per PV string 1-4
  P_pv_min = [1.5   1.5   1.5   1.5];
  P_pv_nom = [7.5   7.5   7.5   7.5];
  ...
  for(i=1:N_daily_dispatch)
   ...
   %Initialize active power generation for wind
    ...
    mpc_substation_system13.bus(1,3) = P_wt_total(1,:) + sum(P_pv(1,:),2);
    
    %initialize normal active power generation for solar 
    mpc_substation_system13.gen(2:5,2) = P_pv(1,:);
...
% a = 0.08;                    %Percentage of standard deviation, from papers around from 8% till 2% 
% 
...
for(i=1:(N_daily_dispatch))
    if(i <= (N_daily_dispatch/10))
        Q_mean_pv1(i,j) = -10*(Q_dem_tot_pv/ N_strings_pv)/10 ;
    elseif ((i > (1*N_daily_dispatch/10)) && (i <=(2*N_daily_dispatch/10)))
    for(i=1:(N_daily_dispatch/10))
        Q_mean_pv2(i,j) = -8*(Q_dem_tot_pv/ N_strings_pv)/10;  
    end
    elseif ((i > (2*N_daily_dispatch/10)) && (i <=(3*N_daily_dispatch/10)))
    for(i=1:(N_daily_dispatch/10))
        Q_mean_pv3(i,j) = -6*(Q_dem_tot_pv/ N_strings_pv)/10;  
    end
    elseif ((i > (3*N_daily_dispatch/10)) && (i <=(4*N_daily_dispatch/10)))
    for(i=1:(N_daily_dispatch/10))
        Q_mean_pv4(i,j) = -4*(Q_dem_tot_pv/ N_strings_pv)/10;  
    end
    elseif ((i > (4*N_daily_dispatch/10)) && (i <=(5*N_daily_dispatch/10)))
    for(i=1:(N_daily_dispatch/10))
        Q_mean_pv5(i,j) = -2*(Q_dem_tot_pv/ N_strings_pv)/10;  
    end
    elseif ((i > (5*N_daily_dispatch/10)) && (i <=(6*N_daily_dispatch/10)))
    for(i=1:(N_daily_dispatch/10))
        Q_mean_pv6(i,j) = 0;  
    end
    elseif ((i > (6*N_daily_dispatch/10)) && (i <=(7*N_daily_dispatch/10)))
    for(i=1:(N_daily_dispatch/10))
        Q_mean_pv7(i,j) = 2*(Q_dem_tot_pv/ N_strings_pv)/10;  
    end
    elseif ((i > (7*N_daily_dispatch/10)) && (i <=(8*N_daily_dispatch/10)))
    for(i=1:(N_daily_dispatch/10))
        Q_mean_pv8(i,j) = 4*(Q_dem_tot_pv/ N_strings_pv)/10;  
    end
    elseif ((i > (8*N_daily_dispatch/10)) && (i <=(9*N_daily_dispatch/10)))
    for(i=1:(N_daily_dispatch/10))
        Q_mean_pv9(i,j) = 6*(Q_dem_tot_pv/ N_strings_pv)/10;  
    end
    elseif ((i > (9*N_daily_dispatch/10)) && (i <=(10*N_daily_dispatch/10)))
    for(i=1:(N_daily_dispatch/10))
        Q_mean_pv10(i,j) = 10*(Q_dem_tot_pv/ N_strings_pv)/10;  
    end
end
end
end
...
for(i=1:N_strings_pv) %makes a 1x13
  for(j=1:(N_daily_dispatch/10)) %makes a 96x1
    samples_Q_pv(j,i)= normrnd(Q_mean_pv1(i), a*Q_std_wt(i), [1, 1]); %Creates random samples for Q in 96x13, some values violating the constraints
  end
  for(j=(1*N_daily_dispatch/10):(2*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv2(i), a*Q_std_wt(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(2*N_daily_dispatch/10):(3*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv3(i), a*Q_std_wt(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(3*N_daily_dispatch/10):(4*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv4(i), a*Q_std_wt(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(4*N_daily_dispatch/10):(5*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv5(i), a*Q_std_wt(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(5*N_daily_dispatch/10):(6*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv6(i), a*Q_std_wt(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(6*N_daily_dispatch/10):(7*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv7(i), a*Q_std_wt(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(7*N_daily_dispatch/10):(8*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv8(i), a*Q_std_wt(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(8*N_daily_dispatch/10):(9*N_daily_dispatch/10))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv9(i), a*Q_std_wt(i), [1, 1]); %Creates random samples for Q 
  end
  for(j=(9*N_daily_dispatch/10):(N_daily_dispatch))
       samples_Q_pv(j,i)= normrnd(Q_mean_pv10(i), a*Q_std_wt(i), [1, 1]); %Creates random samples for Q 
  end
end
...
while(stop == 0)
   ...
   %for (k=1:N_branches_shunt)
        ...
        for(a=1:4)
        mpc_substation_system13.gen(P_pv_idx(a),3) = samples_Q_pv(i,a);
        end
        %mpc_substation_system13.bus(1,4) = sum(samples_Q_tot(i,:));
        %initialize active power generation
