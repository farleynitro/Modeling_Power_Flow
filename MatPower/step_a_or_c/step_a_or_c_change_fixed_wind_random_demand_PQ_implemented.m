...
N_daily_dispatch = 1000; %total available wind dispatch profiles for 3 hours
N_strings = 13; %total Wind Turbine Generators strings
  ...
  for (i=1:N_daily_dispatch) 
    v_w(i,:) = 23 ; %nominal wind speed in m/s
  end  
...
for(i=1:N_strings) %makes a 1x13
  for(j=1: N_daily_dispatch) %makes a 96x1
    samples_Q(j,i)= normrnd(Q_mean(i), Q_std(i),[1,1]); %Creates random samples for Q in 96x13, some values violating the constraints
  end
end
